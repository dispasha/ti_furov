...
app.post('/tasklists/:tasklistId/tasks', async (request, response) => {
  ...
  const { taskName } = request.body[0];
  const { taskTime } = request.body[1];
  ...
  tasklists[tasklistId].tasks.push([taskName, taskTime]);
  tasklists[tasklistId].tasks.sort((a,b) => { 
    if (a[1] < b[1]) return -1;
...
app.put('/tasklists/:tasklistId/tasks/:taskId', async (request, response) => {
  const { newTaskName } = request.body[0];
  const { newTaskTime } = request.body[1];
  const tasklistId = Number(request.params.tasklistId);
  ...
  tasklists[tasklistId].tasks[taskId][0] = newTaskName;
  tasklists[tasklistId].tasks[taskId][1] = newTaskTime;
  tasklists[tasklistId].tasks.sort((a,b) => { 
    if (a[1] < b[1]) return -1;
    else if (a[1] > b[1]) return 1;
    else return 0})
...
app.delete('/tasklists/:tasklistId/tasks/:taskId', async (request, response) => {
  ...
  const deletedTaskName = tasklists[tasklistId].tasks[taskId][0];
...
app.patch('/tasklists/:tasklistId', async (request, response) => {
  ...
  tasklists[fromTasklistId].tasks.sort((a,b) => { 
    if (a[1] < b[1]) return -1;
    else if (a[1] > b[1]) return 1;
    else return 0});
  tasklists[toTasklistId].tasks.sort((a,b) => { 
      if (a[1] < b[1]) return -1;
      else if (a[1] > b[1]) return 1;
      else return 0});
  await writeData(tasklists);
